...
int CPXPUBLIC add_BENDERS_sec_callback_relaxation_concorde(
    ...
    doit_fn_input *data = (doit_fn_input*)calloc(1, sizeof(struct doit_fn_input_t));
    data->nedges = nedges;
    ...
    //avoid zero edges: have to know size in advance?
    ...
    if(ncomps == 1)
    {
        if (CCcut_violated_cuts(nedges, ncols, elist, xstar, 2 - epsilon, doit_fn_concorde, (void *)data))
...
/*
    ...
    printf("nnz = %d\n:", nnz);
    printf("value (%d):\n", cutcount * (cutcount - 1) / 2); for(int i=0; i< cutcount * (cutcount - 1) / 2; i++) printf("%f\n", value[i]);
    printf("index (%d):\n", cutcount * (cutcount - 1) / 2); for(int i=0; i< cutcount * (cutcount - 1) / 2; i++) printf("%d\n", index[i]);
*/
    //FIX: segmentation fault: why?
    if (CPXcallbackaddusercuts(data->context, 1, nnz, &rhs, &sense, &izero, index, value, &purgeable, &local))
        print_error("CPXcallbackaddusercuts() error"); 
    //todo debug: print cut
