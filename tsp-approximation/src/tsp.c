...
instance generate_random_instance(int id, int nnodes, int box_size) {
    ...
    //random pick from an uniforme distribution the number of nodes in range [9/10(nnodes), 11/10(nnodes)]
    double randomFluttuation = (double)rand() / (double)RAND_MAX - 0.5;
    randomFluttuation = randomFluttuation * nnodes / 5;
    nnodes += (int) randomFluttuation;
